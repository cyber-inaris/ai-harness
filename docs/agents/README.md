# Agents

Runbooks for AI agents that operate this harness.

Current direction:

```text
Hermes = Telegram-facing control plane
LangGraph = primary task runtime
CrewAI = optional worker backend
```

Current notes:

- [Agent VPS Setup Runbook](agent-vps-setup.md)
- [LangGraph Runtime](langgraph-runtime.md)
- [Hermes Modes And Skills](hermes-modes-and-skills.md)
- [Notion Task Board](notion-task-board.md)
- [Hermes Runtime Setup](hermes-runtime-setup.md)
- [CrewAI MVP Setup](crewai-mvp-setup.md)
- [CrewAI Orchestration](crewai-orchestration.md)
- [Hermes Agent On VPS](hermes-agent.md)
- [Telegram Command Router](telegram-command-router.md)
